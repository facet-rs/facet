...
impl FormatSuite for MsgPackSlice {
    ...
    // MsgPack is a binary format - test cases need binary input, not JSON strings.
    // While MsgPack is self-describing (unlike ASN.1), the suite uses JSON string
    // inputs which won't parse as valid MsgPack.
