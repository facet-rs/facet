...
- Real-workspace full solve remains **64 / 863 Cargo-resolved package-version rows diffed against Cargo.lock** at the largest measured ring. The machine-lane `molten handle -1` failure is gone, and the root prerelease empty-solve is fixed; the 145-member solve hit the wall-clock frontier at 180.020s and was stopped before direct-dep expansion.
...
| Real workspace member+direct deps | 145 members, 1,124 direct deps | required workspace-known direct clauses implemented but not measured after the 145-member wall-clock timeout | next frontier after full member-only solve |
...
| `cargo check --workspace --all-targets` | passed; 2.52s |
| `cargo nextest run -p vix --features real-process` | passed; 206 passed, 18 skipped, 1 leaky, 95.818s |
| `cargo clippy --workspace --all-targets --all-features -- -D warnings` | passed; 2.60s |
...
The next ring is no longer blocked by the prerelease/root-clause empty solve. The current frontier is runtime growth before full member-only ring completion:
