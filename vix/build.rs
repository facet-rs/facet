...
impl Decode {
    ...
    fn lower_fn(self) -> &'static str {
        match self {
            ...
            Decode::Unit => panic!("unit leaves carry no value"),
        }
    }
...
impl<'a> Model<'a> {
    ...
    fn resolve_shape(&mut self, kind: &str, fname: &str, alts: Vec<Alt>) -> Shape {
        ...
        if rules.len() == 1 {
            ...
            return if let Some(e) = self.hidden_enum(r) {
                ...
                Shape::Leaf(r.clone(), self.leaf_decode(r))
            };
        }
    ...
    fn variant_payload(&self, kind: &str) -> String {
        if self.is_struct_kind(kind) {
            ...
            match self.leaf_decode(kind) {
                Decode::Unit => String::new(),
                d => format!("({})", d.rust_type()),
            }
        }
    }
    ...
    fn variant_lower(&self, enum_name: &str, kind: &str, c: &str) -> String {
        ...
        if self.is_struct_kind(kind) {
            ...
            match self.leaf_decode(kind) {
                Decode::Unit => format!("{enum_name}::{variant}"),
                d => format!("{enum_name}::{variant}({})", d.lower(c)),
            }
        }
    }
    ...
    fn field_type(&self, f: &FieldDef) -> String {
        let base = match &f.shape {
            ...
            Shape::Leaf(kind, decode) => match decode {
                Decode::Unit => panic!(
                    "field `{}` holds unit leaf `{kind}` — carry nothing instead",
                    f.grammar_name
                ),
                d => d.rust_type().to_string(),
            },
    ...
    fn field_lower_fn(&self, f: &FieldDef) -> String {
        match &f.shape {
            ...
            Shape::Leaf(_, decode) => decode.lower_fn().to_string(),
