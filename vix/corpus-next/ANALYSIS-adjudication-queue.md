...
| **A5** | **Exec observers** | `exec-observers.md` (whole note); *"trees don't replace observers"*. Open Q2 (capability default vs per-call) has a stated lean. | **2/3 ports.** `crate.vix:1211, 1304` (`build_script!` then re-parse stdout after the fact at `:1411`); `rodin.vix:438, 447` (rustc `--print cfg` routed **through a file in a Tree** just to read stdout). | Take the lean (capability-level default, per-call override). The `rodin.vix:438` `--stdout {p"cfg.stdout"}` idiom is the strongest evidence in the corpus that stdout has no home. | design pass |
