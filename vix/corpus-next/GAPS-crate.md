...
- New: 1630 lines, 135 functions, 22 exported functions.
- Delta: -430 lines, -59 functions; exported entrypoints preserved.
...
- `vix/corpus-next/crate.vix:712`, `vix/corpus-next/crate.vix:725`, `vix/corpus-next/crate.vix:807`, `vix/corpus-next/crate.vix:815`, `vix/corpus-next/crate.vix:1613`, and nearby path sites use `p""` segments plus `/` joins instead of embedded slash strings.
...
- `vix/corpus-next/crate.vix:220`: a workspace-table edition inside `workspace.package.edition` is treated as malformed via a missing map lookup, because the designed decode shape has no typed error construction. PROPOSAL: ratify an expression-level typed error/result surface for impossible decoded shapes.
...
- `vix/corpus-next/crate.vix:1370`: directive dispatch is still a nested string ladder. PROPOSAL: ratify string-literal match tables or const maps for string-key dispatch.
- `vix/corpus-next/crate.vix:1411`: stdout line parsing keeps the old recursive string walk because no `String::lines()`/split iterator is banked. PROPOSAL: ratify `String::lines() -> [String]` or a pure split combinator.
- `vix/corpus-next/crate.vix:1563`: proc-macro dynamic-library naming still uses local target OS matching. PROPOSAL: add a Cargo/rustc helper for proc-macro dylib filenames per host target.

## Duplication verdict

The build lane's duplication mostly dissolves. The original combinatorial helper family is replaced by one `RustUnit` builder plus small adapters from the lock, resolved-graph, solution-graph, build-script, and proc-macro domains. The remaining duplication is concentrated in two places: the four emit arms in `compile_rust_unit` (`vix/corpus-next/crate.vix:596`) and Cargo-domain boilerplate the book does not yet model (`vix/corpus-next/crate.vix:1178`, `vix/corpus-next/crate.vix:1370`). Those are real language/library gaps, not just local refactor misses.
