...
"#;
    ...
    fn crate_sample_modules() -> BTreeMap<String, String> {
        BTreeMap::from([
            (
                "root".to_owned(),
            ...
            (
                "rodin".to_owned(),
                include_str!("../../../rodin/rodin.vix").to_owned(),
            ),
        ])
    }
...
"#;
    ...
    fn crate_vix_fake_rustc_builds_lib_on_the_machine() {
        let modules = crate_sample_modules();
        for lane in lanes() {
            let mut machine = load_modules_with_lane("root", modules.clone(), lane);
            let target = machine.linux_target_handle();
            let crate_tree =
