...
use super::driver::{
    ...
    MoltenStats, NATIVE_OPTION_UNWRAP_NONE_HOST, OCI_DOC_HOST, OPTION_CONSTRUCT_HOST,
            ...
            "get" => {
                if self.tables.schemas.is_list(&receiver.schema) {
                    ...
                    if !schema_is_inline_word(&self.tables.schemas, &elem_schema) {
                        return Err(format!(
                            "Array.get native path is pinned for aggregate element {elem_schema}"
                        ));
                    }
                    let index = self.method_arg(&call.args.args[0], Some("Int"))?;
...
"#;
    ...
    #[test]
    fn aggregate_array_get_remains_pinned_until_materialization_exists() {
        let src = r#"
struct Domain {
    active: Bool,
}
...
"#;
        let err = match Machine::load(src) {
            Ok(_) => panic!("aggregate get should remain pinned"),
            Err(err) => err,
        };
        assert!(err.contains("Array.get native path is pinned"), "{err}");
    }
