...
fn collect_free_idents_expr(
    ...
    match expr {
        ...
        ast::Expr::Exec(exec) => {
            // The capability tag (`echo`, `sh`) is a value binding the command
            // references; the template body carries no identifiers yet.
            if !bound.contains(&exec.capability.value) {
                out.insert(exec.capability.value.clone());
            }
        }
...
fn expr_references_name(expression: &ast::Expr, name: &str) -> bool {
    match expression {
        ...
        // The capability tag names a value binding the command references.
        ast::Expr::Exec(exec) => exec.capability.value == name,
