...
fn bench_vix(
    ...
    let total = i64::try_from(token_count(terms)).map_err(|_| "token count does not fit Int")?;
    let start = Instant::now();
    let mut checksum_sum = 0;
    for seed in 0..runs {
        let value = machine.demand_i64("parse_entry", vec![total, seed])?;
        if value != expected {
            return Err(format!(
...
fn vix_source(_terms: usize) -> String {
    r#"
    ...
    let next_lookahead = match next_remaining {
        0 => 2,
        _ => match lookahead {
            0 => 1,
            1 => 0,
    ...
    match action {
        0 => reduces,
        101 => parse(remaining, (stack.pop().1).push(1), 1, lookahead, reduces + 1),
        102 => parse(
            remaining,
            (((stack.pop().1).pop().1).pop().1).push(1),
            1,
            lookahead,
            reduces + 1
        ),
...
pub fn parse_entry(total: Int, seed: Int) -> Int {
    parse(total - 1, [0], 0, 0, seed) - seed
}
    "#
    .to_string()
}
        ...
        for lane in test_lanes() {
            ...
            assert_eq!(
                machine
                    .demand_i64(
                        "parse_entry",
                        vec![i64::try_from(token_count(terms)).unwrap(), 7]
                    )
                    .unwrap(),
                expected,
                "{lane:?}"
