...
mod tests {
    ...
    fn test_attributes_are_parsed() {
        ...
        let svg: Svg = facet_xml_legacy::from_str(xml).unwrap();
    ...
    fn test_svg_float_tolerance() {
        ...
        let c: Svg = facet_xml_legacy::from_str(c_svg).unwrap();
        let rust: Svg = facet_xml_legacy::from_str(rust_svg).unwrap();
