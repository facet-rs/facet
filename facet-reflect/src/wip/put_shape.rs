        ...
        if frame.shape != src_shape {
            ...
            if let Some(inner_fn) = frame.shape.inner {
                ...
                if src_shape == inner_shape {
                    ...
                    if let Some(try_from_fn) = (frame.shape.vtable.sized().unwrap().try_from)() {
            ...
            if let Some(try_from) = (frame.shape.vtable.sized().unwrap().try_from)() {
                ...
                if let Some(fields) = fields {
                    ...
                    for (i, field) in fields.iter().enumerate() {
                        if !frame.istate.fields.has(i) {
                            ...
                            if let Some(try_from) =
                                (field.shape().vtable.sized().unwrap().try_from)()
                            {
                                debug!(
            ...
            match frame.shape.ty {
                Type::User(UserType::Struct(sd)) => {
                    for (i, field) in sd.fields.iter().enumerate() {
                        if frame.istate.fields.has(i) {
                            if let Some(drop_fn) =
                                (field.shape().vtable.sized().unwrap().drop_in_place)()
                            {
                                unsafe {
                ...
                _ => {
                    ...
                    if frame.istate.fields.is_any_set() {
                        ...
                        if let Some(drop_fn) = (frame.shape.vtable.sized().unwrap().drop_in_place)()
                        {
