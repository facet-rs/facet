...
#[test]
fn build_with_invariants() -> Result<(), IPanic> {
...
#[test]
fn build_struct_with_enum_field() -> Result<(), IPanic> {
    #[derive(Facet, PartialEq, Debug)]
    #[repr(C)]
    enum Range {
        Low(u8),
    ...
    partial = partial.begin_nth_field(0)?;
    partial = partial.set(25u8)?;
    partial = partial.end()?;
    partial = partial.end()?;
    let value: ValidatedRange = partial.build()?.materialize::<ValidatedRange>()?;
...
#[test]
fn build_nested_with_invariants() -> Result<(), IPanic> {
    #[derive(Facet, PartialEq, Debug)]
    ...
    partial = partial.end()?;
    let container: Container = partial.build()?.materialize::<Container>()?;
