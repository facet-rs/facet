...
```bash
cargo nextest run -p facet-html-diff
```

Roundtrip tests live in `tests/roundtrip-cases/*.html` with a simple format:
...
The fuzzer generates random HTML trees and verifies the roundtrip invariant:
