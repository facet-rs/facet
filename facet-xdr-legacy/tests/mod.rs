...
use facet_xdr_legacy::{deserialize, to_vec};
...
fn test_option_is_the_same_as_vec() {
    ...
    let option_serialization = facet_xdr_legacy::to_vec(&Some(inner)).unwrap();
    let vec_serialization = facet_xdr_legacy::to_vec(&vec![inner]).unwrap();
    assert_eq!(
