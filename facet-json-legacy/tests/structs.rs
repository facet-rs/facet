...
fn json_read_struct_twofields() {
    ...
    let s: TestStruct = facet_json_legacy::from_str(json).expect("Failed to parse JSON"); // Changed to expect to avoid ? causing implicit Result<()>
...
fn test_from_json_with_nested_structs() {
    ...
    let test_struct: OuterStruct = facet_json_legacy::from_str(json).expect("Failed to parse JSON"); // Changed to expect to avoid ? causing implicit Result<()>
...
fn test_reading_flat_structs() {
    ...
    let actual1: Outer = facet_json_legacy::from_str(
    ...
    let actual2: Outer =
        facet_json_legacy::from_str(r#"{"name":"test2","val":2,"Variant2":"ccc"}"#)
    ...
    let actual3: Outer =
        facet_json_legacy::from_str(r#"{"name":"test3","val":3,"Variant3":["ddd","eee"]}"#)
...
fn test_writing_flat_structs() {
    ...
    let actual1 = facet_json_legacy::to_string(&Outer {
        name: "test1",
        struct_: InnerStruct { val: 1 },
        enum_: InnerEnum::Variant1 {
            field1: "aaa".to_string(),
    ...
    let actual2 = facet_json_legacy::to_string(&Outer {
        name: "test2",
        struct_: InnerStruct { val: 2 },
        enum_: InnerEnum::Variant2("ccc".to_string()),
    });
    ...
    let actual3 = facet_json_legacy::to_string(&Outer {
        name: "test3",
        struct_: InnerStruct { val: 3 },
        enum_: InnerEnum::Variant3("ddd".to_string(), "eee".to_string()),
    });
