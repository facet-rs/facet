...
fn parse_ok() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parsed_ok = facet_json_legacy::from_str::<String>(OK_JSON)?;
...
fn parse_fail() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parsed_fail = facet_json_legacy::from_str::<String>(FAIL_JSON)?;
...
fn test_unicode_escapes() -> Result<(), Box<dyn std::error::Error>> {
    for (input, expected) in UNICODE_TEST_CASES {
        ...
        let parsed = facet_json_legacy::from_str::<String>(input)?;
...
fn test_invalid_surrogate_sequences() {
    ...
    let result = facet_json_legacy::from_str::<String>("\"\\ud83c\"");
    assert!(
        result.is_err(),
    ...
    let result = facet_json_legacy::from_str::<String>("\"\\ud83c\\u0041\"");
    assert!(
        result.is_err(),
    ...
    let result = facet_json_legacy::from_str::<String>("\"text\\ud83c\"");
    assert!(
        result.is_err(),
...
fn test_bmp_characters_after_surrogate_fix() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parsed = facet_json_legacy::from_str::<String>("\"\\u0041\\u0042\\u0043\"")?;
    assert_eq!(parsed, "ABC");

    // Non-ASCII BMP characters (Chinese)
    let parsed = facet_json_legacy::from_str::<String>("\"\\u4e2d\\u6587\"")?;
    assert_eq!(parsed, "中文");

    // Greek letters
    let parsed = facet_json_legacy::from_str::<String>("\"\\u03b1\\u03b2\\u03b3\"")?;
...
fn test_ascii_control_character_serialization() -> Result<(), Box<dyn std::error::Error>> {
    for (input_char, expected_json) in CONTROL_CHAR_TEST_CASES {
        ...
        let serialized = facet_json_legacy::to_string(&input_string);
        assert_eq!(
            &serialized,
            expected_json,
...
fn test_control_characters_in_mixed_strings() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (input, expected) in test_cases {
        let serialized = facet_json_legacy::to_string(input);
...
fn test_control_character_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (input_char, _) in CONTROL_CHAR_TEST_CASES {
        let input_string = input_char.to_string();
        let serialized = facet_json_legacy::to_string(&input_string);
        let deserialized: String = facet_json_legacy::from_str(&serialized)?;
...
fn test_all_escape_sequences_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for original in test_strings {
        let serialized = facet_json_legacy::to_string(&original);
        let deserialized: String = facet_json_legacy::from_str(&serialized)?;
