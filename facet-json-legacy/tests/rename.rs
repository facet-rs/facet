use facet::Facet;
use facet_json_legacy::{JsonErrorKind, from_str, to_string};
...
fn test_field_rename_ignore_extra_fields() {
    ...
    let user: User = facet_json_legacy::from_str(json).unwrap();
...
fn test_field_rename_missing_required_error() {
    ...
    let result = facet_json_legacy::from_str::<Required>(json);
