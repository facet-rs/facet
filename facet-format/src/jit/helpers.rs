...
// =============================================================================
// JSON Format JIT Helpers (Tier-2)
// =============================================================================
//
// These helpers implement JSON parsing operations for direct byte-level parsing.
// They are called by JIT-compiled code generated by format_compiler.rs.
...
pub unsafe extern "C" fn json_jit_seq_begin(
    ...
    if pos >= len {
        return JsonJitPosError {
            new_pos: pos,
            error: json_jit_error::UNEXPECTED_EOF,
        };
    }
    ...
    if byte != b'[' {
        return JsonJitPosError {
            new_pos: pos,
            error: json_jit_error::EXPECTED_ARRAY_START,
        };
    }
...
pub unsafe extern "C" fn json_jit_seq_next(
    ...
    if byte == b',' {
        ...
        JsonJitPosError {
            new_pos: pos,
            error: json_jit_error::EXPECTED_COMMA_OR_END,
        }
    }
}
...
pub unsafe extern "C" fn json_jit_parse_bool(
    ...
    JsonJitPosValueError::new(pos, false, json_jit_error::EXPECTED_BOOL)
}
