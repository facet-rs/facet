    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    async fn recv_timeout(&mut self, timeout: Duration) -> io::Result<Option<Message>> {
        moire::timeout!(timeout, self.recv(), "ws.recv")
            .await
            .unwrap_or(Ok(None))
    }
...
mod tests {
    ...
    async fn websocket_hello_exchange() {
        ...
        let server_handle = moire::spawn_tracked!("roam_ws_test_server", async move {
        ...
        // Spawn client driver
        moire::spawn_tracked!("roam_ws_test_client_driver", client_driver.run());

        // Server should also succeed
        let (server_handle_result, _server_incoming, server_driver) =
            server_handle.await.unwrap().unwrap();
        moire::spawn_tracked!("roam_ws_test_server_driver", server_driver.run());
