...
pub async fn handle_websocket(
    ...
    let (outgoing_tx, mut outgoing_rx) = peeps::channel!("ws_outgoing", 256);
...
async fn setup_streaming_call(
    ...
    for &roam_channel_id in roam_to_ws_tx_map.keys() {
        let (tx, rx) = peeps::channel!("ws_roam_incoming", 256);
    ...
    // This is critical - channels must be registered before any data messages arrive
    {
        ...
        for (&ws_channel_id, &(roam_channel_id, elem_shape)) in &ws_to_roam_rx_map {
            let (ws_data_tx, _ws_data_rx) = peeps::channel!("ws_data", 256);
    ...
    let (response_tx, response_rx) = peeps::oneshot!("ws_streaming_response");
