...
#[tokio::test]
async fn abort_cancels_volatile_handler() {
    ...
    let server = tokio::spawn({
        ...
        async move {
            let (s, _sh) = vox::acceptor_on(server_link)
                .establish::<vox::NoopClient>(BlockerDispatcher::new(BlockingService {
                    was_cancelled,
                }))
                .await
    ...
    let (client, _sh) = vox::initiator_on(client_link, vox::TransportMode::Bare)
        .establish::<BlockerClient>(())
        .await
        .expect("client establish");

    let _server_guard = server.await.expect("server task");

    // Spawn the call and then abort it.
    let call = tokio::spawn(async move { client.block().await });
    tokio::time::sleep(Duration::from_millis(50)).await;
    call.abort();

    // Wait for the cancellation to propagate to the server handler.
    for _ in 0..20 {
    ...
    assert!(
        ...
        "volatile handler should be cancelled when client aborts"
    );
}
