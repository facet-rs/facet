...
async fn open_virtual_connection_and_call() {
    ...
    let (root, session) = vox::initiator_on(client_link, vox::TransportMode::Bare)
        .establish::<vox::NoopClient>(())
        .await
...
async fn reject_virtual_connection() {
    ...
    let (_root, session) = vox::initiator_on(client_link, vox::TransportMode::Bare)
        .establish::<vox::NoopClient>(())
        .await
        .expect("client establish");
...
async fn open_virtual_connection_without_acceptor_is_rejected() {
    ...
    assert!(result.is_err(), "should be rejected without acceptor");
}
