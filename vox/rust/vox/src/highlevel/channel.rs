...
type SendError<T> = vox_rt::sync::mpsc::error::SendError<T>;
#[cfg(target_arch = "wasm32")]
type SendError<T> = vox_rt::sync::mpsc::SendError<T>;
...
impl<L: vox_types::Link + MaybeSend + 'static> ChannelListener<L> {
    ...
    pub fn new(buffer: usize) -> (Self, ChannelListenerSender<L>) {
        let (tx, rx) = vox_rt::sync::mpsc::channel("channel-listener", buffer);
