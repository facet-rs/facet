...
async fn run_server_with_incoming_connections(addr: &str) -> Result<(), String> {
    ...
    peeps_tasks::spawn_tracked("subject_incoming_connections", async move {
...
async fn run_client() -> Result<(), String> {
    ...
    match scenario.as_str() {
        ...
        "sum" => {
            ...
            peeps_tasks::spawn_tracked("subject_sum_sender", async move {
                for i in 1..=5 {
                    debug!("sending {i}");
        ...
        "generate" => {
            ...
            let recv_task = peeps_tasks::spawn_tracked("subject_generate_receiver", async move {
