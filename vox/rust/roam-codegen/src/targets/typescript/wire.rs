...
pub fn generate_wire(config: &WireTypeGenConfig) -> Result<String, Box<dyn std::error::Error>> {
    ...
    // Auto-generate Schema constants for every named type
    for (name, shape) in &named_types {
        let schema = generate_schema(shape);
        out.push_str(&format!(
            "export const {name}Schema: Schema = {schema};\n\n"
        ));
    }
    ...
    for (name, _) in &named_types {
        out.push_str(&format!("  [\"{name}\", {name}Schema],\n"));
    }
    out.push_str("]);\n\n");
