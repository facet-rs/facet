...
fn generate_server_handler(service: &ServiceDetail) -> String {
    ...
    for method in &service.methods {
        ...
        for arg in &method.args {
            ...
            out.push_str(&format!("\t\t\t{arg_name}, err := readString(payload, &off)\n"));
        ...
        // Call handler
        let args_list = method.args.iter()
        ...
        let ret_ty = go_type(&method.return_type);
        if ret_ty == "()" {
            out.push_str(&format!("\t\t\terr = handler.{method_name}(ctx, {args_list})\n"));
            out.push_str("\t\t\tif err != nil {\n");
            out.push_str("\t\t\t\treturn encodeResultErr(err), nil\n");
            out.push_str("\t\t\t}\n");
            out.push_str("\t\t\treturn encodeResultOk(\"\", encodeString), nil\n");
        } else {
            out.push_str(&format!("\t\t\tresult, err := handler.{method_name}(ctx, {args_list})\n"));
            out.push_str("\t\t\tif err != nil {\n");
            out.push_str("\t\t\t\treturn encodeResultErr(err), nil\n");
            out.push_str("\t\t\t}\n");
...
fn generate_runtime_helpers() -> String {
    ...
    out.push_str("\tout = appendUvarint(out, 0) // RapaceError::User\n");
    ...
    out.push_str("\tvar out []byte\n");
    out.push_str("\tout = appendUvarint(out, 1) // Result::Err\n");
    out.push_str("\tout = appendUvarint(out, 1) // RapaceError::UnknownMethod\n");
    out.push_str("\treturn out\n");
    out.push_str("}\n\n");
    ...
    out.push_str("\tvar out []byte\n");
    out.push_str("\tout = appendUvarint(out, 1) // Result::Err\n");
    out.push_str("\tout = appendUvarint(out, 2) // RapaceError::InvalidPayload\n");
    out.push_str("\treturn out\n");
    out.push_str("}\n");
