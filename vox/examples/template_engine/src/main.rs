...
use rapace_template_engine::{
    create_template_engine_dispatcher, create_value_host_dispatcher, ValueHostImpl,
};
...
async fn main() {
    ...
    let channel_id = host_session.next_channel_id();
    let payload = facet_postcard::to_vec(&template.to_string()).unwrap();
    // method_id 1 = render (TemplateEngine's first method)
    match host_session.call(channel_id, 1, payload).await {
        Ok(response) => {
...
mod tests {
    ...
    async fn test_multiple_placeholders() {
        ...
        let channel_id = host_session.next_channel_id();
        let payload = facet_postcard::to_vec(
            &"Hi {{user.name}} from {{site.name}} on {{site.domain}}".to_string(),
        )
        .unwrap();
        let response = host_session.call(channel_id, 1, payload).await.unwrap();
