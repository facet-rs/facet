...
Core runtime library for roam RPC services in Swift.

## Architecture

This package provides the **core runtime components** that are common across all roam services. Service-specific code (method IDs, client stubs, dispatch logic) should be **generated by codegen**, not hand-written.

This follows the same design as:
- **Rust**: `roam-session` crate with `UnaryCaller` trait
- **TypeScript**: `@bearcove/roam-runtime` with `UnaryDispatcher<H>`
...
- `RoamError`: Runtime errors (decode, encode, transport)
