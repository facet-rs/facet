...
import { encodeSchemaPayload } from "./schema_cbor.ts";
...
export class SchemaTracker {
  ...
  buildArgsTranslation(
    ...
    for (let i = 0; i < remoteElements.length; i++) {
      ...
      // Channel args (tx/rx) are handled as channel IDs, not postcard-decoded data.
      // Rust extracts the element type (e.g., i32 for Tx<i32>), but TypeScript keeps
      // the tx/rx schema. Skip translation and use identity for channel args.
      if (resolvedLocal.kind === "tx" || resolvedLocal.kind === "rx") {
        remoteArgSchemas.push(resolvedLocal);
