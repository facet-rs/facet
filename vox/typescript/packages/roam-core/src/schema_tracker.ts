...
export class SchemaSendTracker {
  ...
  prepareSchemas(
    ...
    // Build CBOR SchemaMessagePayload from pre-encoded schema parts.
    const schemaParts = newDeps.map((id) => schemaTable.schemas.get(id)!).filter(Boolean);
    const schemasArray = cborArray(schemaParts);
    ...
    return cborMap([
      ...
      ["method_bindings", cborArray([bindingCbor])],
    ]);
  }
}
