...
fn generate_gungraun_benchmarks(
    ...
    for bench_def in benchmarks {
        output.push_str(&generate_gungraun_benchmark_module(bench_def)?);
    }
...
fn generate_divan_benchmark_module(
    ...
    // JSON data
    let json_content = get_json_content(bench_def)?;
    output.push_str(&format!(
        "    static JSON: &[u8] = br#\"{}\"#;\n\n",
        json_content
    ));
...
fn get_json_content(bench_def: &BenchmarkDef) -> Result<String, Box<dyn std::error::Error>> {
