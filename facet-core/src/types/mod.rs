...
///
/// This is a type alias for `&'static (dyn core::any::Any + Send + Sync)`,
/// used to reduce verbosity in generated code.
...
///
/// The attribute's arguments (tokens) are passed to the getter function at compile time,
/// not stored in this struct. This allows the extension crate to parse and validate
/// the arguments, returning typed data via `get`.
#[derive(Debug)]
pub struct ExtensionAttr {
    ...
    pub key: &'static str,
    /// Function to get the typed, parsed data from the extension crate.
    ///
    /// This function is generated by the derive macro and calls into the
    /// extension crate's attribute function with the parsed tokens.
...
impl ExtensionAttr {
    ...
    ///
    /// Returns `None` if the type doesn't match. This is useful when you
    /// want to handle the case where the attribute might store different types.
    ...
    /// Panics with a helpful message if the downcast fails.
