...
unsafe fn mutex_lock<'a, R: RawMutex, T: Facet<'a>>(opaque: PtrConst) -> Result<LockResult, ()> {
    unsafe {
...
unsafe fn rwlock_read<'a, R: RawRwLock, T: Facet<'a>>(opaque: PtrConst) -> Result<LockResult, ()> {
    unsafe {
...
unsafe fn rwlock_write<'a, R: RawRwLock, T: Facet<'a>>(opaque: PtrConst) -> Result<LockResult, ()> {
    unsafe {
        let rwlock = &*opaque.as_ptr::<RwLock<R, T>>();
