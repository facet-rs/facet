...
fn test_nested_struct_multiple_fields() {
    ...
    assert!(matches!(
        facet_toml_legacy::from_str::<Root>("a = 1").unwrap_err().kind,
        TomlDeErrorKind::ExpectedFieldWithName("nested")
    ));
    assert!(matches!(
        facet_toml_legacy::from_str::<Root>("nested = true")
            .unwrap_err()
            .kind,
...
fn test_ignore_unknown_table_keys() {
    ...
    let result = facet_toml_legacy::from_str::<Manifest>(toml).unwrap();
    assert_eq!(
        result,
        Manifest {
            pkg: PkgTable {
