...
fn test_unit_only_enum() {
    ...
    assert_eq!(
        facet_toml_legacy::from_str::<Root>("value = 'VariantA'").unwrap(),
        Root {
            value: UnitOnlyEnum::VariantA,
        },
    );
    assert_eq!(
        facet_toml_legacy::from_str::<Root>("value = 'VariantB'").unwrap(),
        Root {
    ...
    assert!(matches!(
        facet_toml_legacy::from_str::<Root>("values = true")
            .unwrap_err()
            .kind,
...
fn test_single_value_on_non_unit_enum() {
    ...
    assert!(facet_toml_legacy::from_str::<Root>("value = 'VariantB'").is_err());
}
...
fn test_tuple_enum() {
    ...
    assert_eq!(
        facet_toml_legacy::from_str::<Root>("value = { OneField = 0.5 }").unwrap(),
        Root {
            value: WithTupleVariants::OneField(0.5)
        },
    );
    assert_eq!(
        facet_toml_legacy::from_str::<Root>(
            r#"
            [value.TwoFields]
            0 = true
            1 = 1
...
fn test_struct_enum() {
    ...
    assert_eq!(
        facet_toml_legacy::from_str::<Root>("value.OneField.one = 0.5").unwrap(),
        Root {
...
fn test_nested_struct_enum() {
    ...
    assert_eq!(
        facet_toml_legacy::from_str::<Root>("value.OneField.one.NestedOneField.nested_one = 0.5").unwrap(),
        Root {
...
fn test_enum_root() {
    ...
    assert_eq!(
        facet_toml_legacy::from_str::<Root>("A.value = 1").unwrap(),
        Root::A { value: 1 },
    );
    assert_eq!(facet_toml_legacy::from_str::<Root>("B = 2").unwrap(), Root::B(2));
    assert_eq!(facet_toml_legacy::from_str::<Root>("[C]").unwrap(), Root::C);
}
