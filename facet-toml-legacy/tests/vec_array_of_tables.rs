...
fn test_nested_unit_struct_vec() {
    ...
    let serialized = facet_toml_legacy::to_string(&root).unwrap();
    println!("Serialized nested unit struct vec:\n{serialized}");

    // This will panic because the 'unit' field is not present in the serialized TOML
    let _deserialized: Root = facet_toml_legacy::from_str(&serialized).unwrap();
}
...
fn test_nested_struct_multiple_fields_vec() {
    ...
    let deserialized: RootWithNested = facet_toml_legacy::from_str(&serialized).unwrap();
...
"#;
    ...
    let deserialized: RootWithNested = facet_toml_legacy::from_str(toml).unwrap();
