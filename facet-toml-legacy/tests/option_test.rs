...
fn test_simple_option_some() {
    ...
    let result: Result<SimpleOption, _> = facet_toml_legacy::from_str(toml_str);
    if let Err(e) = &result {
        eprintln!("Error: {e}");
    }
    assert!(result.is_ok());
...
"#;
    ...
    let result: Result<MultipleOptions, _> = facet_toml_legacy::from_str(toml_str);
    if let Err(e) = &result {
        eprintln!("Error: {e}");
    }
    assert!(result.is_ok());
...
"#;
    ...
    let result: Mixed = facet_toml_legacy::from_str(toml_str).unwrap();
