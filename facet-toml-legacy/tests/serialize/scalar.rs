...
use facet_toml_legacy::TomlSerError;
...
fn test_u64_out_of_range() {
    ...
    assert!(matches!(
        facet_toml_legacy::to_string(&Root { value: u64::MAX }).unwrap_err(),
...
fn test_u128_out_of_range() {
    ...
    assert!(matches!(
        facet_toml_legacy::to_string(&Root { value: u128::MAX }).unwrap_err(),
        TomlSerError::InvalidNumberToI64Conversion { .. }
    ));
}
...
fn test_i128_out_of_range() {
    ...
    assert!(matches!(
        facet_toml_legacy::to_string(&Root { value: i128::MAX }).unwrap_err(),
        TomlSerError::InvalidNumberToI64Conversion { .. }
    ));
}
