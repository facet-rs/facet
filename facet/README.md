...
// Set field values by name using the slot API
// Each operation returns the parent context after setting the value
let poke = poke.field_by_name("foo").unwrap().set(42u64).unwrap().into_struct_uninit();
let poke = poke
    .field_by_name("bar")
    .unwrap()
...
let foo_bar = poke.build::<FooBar>(Some(guard)).unwrap();
