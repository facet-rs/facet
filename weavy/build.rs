...
#[cfg(feature = "jit")]
const BLAKE3_SYMBOLS: &[&str] = &["weavy_blake3_hash"];
...
fn emit_blake3_native(out: &Path, generated: &Path) {
    ...
    let bytes = fs::read(&obj).unwrap();
    let stencil = extract_stencil(&bytes, BLAKE3_SYMBOLS, "weavy_blake3_hash", "weavy_cont");
    let mut s = String::new();
    s.push_str("// Generated by build.rs from stencils/blake3_hash.rs (BLAKE3 spike).\n");
    emit(
        &mut s,
        "HASH",
        "`weavy_blake3_hash`: hash power-of-two 1KiB chunks from frame pointers.",
        &stencil,
    );
    emit_cont(&mut s, "HASH_CONT", "HASH", &stencil);
    fs::write(generated, s).unwrap();
}
