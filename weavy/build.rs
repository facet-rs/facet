...
fn main() {
    #[cfg(feature = "jit")]
    {
        ...
        let blake3_generated = out.join("weavy_blake3_stencils.rs");
        ...
        if native_copy_patch_target(&target_os, &target_arch) {
            ...
            if target_arch == "aarch64" {
                emit_blake3_native(&out, &blake3_generated);
            } else {
                emit_blake3_empty(&blake3_generated);
            }
        } else {
...
#[cfg(feature = "jit")]
const BLAKE3_SYMBOLS: &[&str] = &[
    "weavy_blake3_hash_small",
    "weavy_blake3_fold_parent",
    "weavy_blake3_hash_batch",
];

#[cfg(feature = "jit")]
fn emit_blake3_empty(generated: &Path) {
    fs::write(
        generated,
        "pub const HASH_SMALL: &[u8] = &[];\n\
         pub const HASH_SMALL_CONT: &[usize] = &[];\n\
         pub const FOLD_PARENT: &[u8] = &[];\n\
         pub const FOLD_PARENT_CONT: &[usize] = &[];\n\
         pub const HASH_BATCH: &[u8] = &[];\n\
         pub const HASH_BATCH_CONT: &[usize] = &[];\n",
    )
    .unwrap();
}

#[cfg(feature = "jit")]
fn emit_blake3_native(out: &Path, generated: &Path) {
    println!("cargo:rerun-if-changed=stencils/blake3_hash.rs");
    println!("cargo:rerun-if-changed=stencils/blake3_neon_core.rs");

    let target = env::var("TARGET").expect("TARGET set by cargo");
    let obj = out.join("weavy_blake3_stencils.o");
    let src = Path::new("stencils/blake3_hash.rs");
    let rustc = env::var("RUSTC").unwrap_or_else(|_| "rustc".to_string());
    ...
    if !tailcall {
        assert!(
            compile_object(&rustc, &[], src, &obj, &target, false),
            "rustc failed to compile Weavy BLAKE3 stencils"
        );
    }

    let bytes = fs::read(&obj).unwrap();
    let mut s = String::new();
    s.push_str("// Generated by build.rs from stencils/blake3_hash.rs (BLAKE3 NEON spike).\n");
    for (name, sym, doc) in [
        (
            "HASH_SMALL",
            "weavy_blake3_hash_small",
            "`weavy_blake3_hash_small`: hash one <=1KiB value via AArch64 NEON.",
        ),
        (
            "FOLD_PARENT",
            "weavy_blake3_fold_parent",
            "`weavy_blake3_fold_parent`: fold two 32-byte hashes via one parent compression.",
        ),
        (
            "HASH_BATCH",
            "weavy_blake3_hash_batch",
            "`weavy_blake3_hash_batch`: hash N cache-resident equal-size values.",
        ),
    ] {
        let stencil = extract_stencil(&bytes, BLAKE3_SYMBOLS, sym, "weavy_cont");
        emit(&mut s, name, doc, &stencil);
        emit_cont(&mut s, &format!("{name}_CONT"), name, &stencil);
    }
    fs::write(generated, s).unwrap();
}
