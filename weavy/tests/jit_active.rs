//! Focused tests for Weavy's build predicate: `jit_active = feature_enabled &&
//! target_supports_copy_patch`, over an explicit OS+arch matrix (not an
//! OS-only check — copypatch's native backend is arch-specific too, so e.g.
//! linux-aarch64 or macos-x86_64 must stay off even though their OS alone
//! would pass an OS-only gate).
//!
...
fn jit_active_on_supported_os_arch_pairs() {
    assert!(jit_config::jit_active(true, "macos", "aarch64"));
    assert!(jit_config::jit_active(true, "linux", "x86_64"));
}
...
fn jit_inactive_on_unsupported_arch_even_with_a_supported_os() {
    ...
    assert!(!jit_config::jit_active(true, "macos", "x86_64"));
    assert!(!jit_config::jit_active(true, "linux", "aarch64"));
}
...
fn jit_inactive_on_unsupported_os() {
    for (os, arch) in [
        ...
        assert!(!jit_config::jit_active(true, os, arch), "{os}-{arch}");
    }
}

#[test]
fn jit_inactive_when_feature_off_even_on_supported_targets() {
    assert!(!jit_config::jit_active(false, "macos", "aarch64"));
    assert!(!jit_config::jit_active(false, "linux", "x86_64"));
}
