...
//! exercise the substrate on both backends. Generalizing the operand to a
//! tagged `Value` and wiring a `suspend` node into [`crate::ir`] are the
//! documented follow-ups; the suspend/resume PROTOCOL here is the reusable part.
