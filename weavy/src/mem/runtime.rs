...
/// A checked-out pair slot from [`RawPairBuilder`].
#[derive(Clone, Copy, Debug)]
pub struct RawPairSlot {
    ptr: *mut u8,
}

/// Growable engine-owned raw storage for directly filling `(K, V)` pairs.
///
/// Callers reserve the next pair slot, initialize its key and value, then mark
/// it initialized. If decoding fails before adoption, dropping the builder
/// calls the supplied drop thunks for initialized keys and values and frees the
/// raw allocation. After a map handle adopts the pair values through a
/// `from_pair_slice`-style thunk, call [`adopt`](Self::adopt) so the builder
/// does not touch the moved-out values while still freeing the raw storage.
#[derive(Debug)]
pub struct RawPairBuilder {
    ptr: *mut u8,
    len: usize,
    cap: usize,
    pair_layout: Layout,
    ...
    key_drop_ctx: *const (),
    key_drop: DropThunk,
    value_drop_ctx: *const (),
    value_drop: DropThunk,
}

impl RawPairBuilder {
    /// Create an empty builder for pairs with `pair_layout`.
    #[must_use]
    pub fn new(
        pair_layout: Layout,
        value_offset: usize,
        ...
        let empty = empty_array_layout(pair_layout.align());
        Self {
            ptr: allocate_or_dangling(empty),
            len: 0,
            cap: 0,
    ...
    /// The start of the raw pair storage.
    #[must_use]
    pub fn ptr(&self) -> *mut u8 {
        self.ptr
    }

    /// Number of initialized pairs.
    #[must_use]
    pub fn len(&self) -> usize {
        self.len
    }

    /// Whether there are no initialized pairs.
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.len == 0
    }

    /// Number of pair slots allocated.
    #[must_use]
    pub fn cap(&self) -> usize {
        self.cap
    }

    /// Byte stride between pairs.
    #[must_use]
    pub fn stride(&self) -> usize {
        self.pair_layout.size()
    }

    /// Reserve and return the next uninitialized pair slot.
    pub fn next_uninit_pair(&mut self) -> Result<RawPairSlot, RawAllocError> {
        let required = self.len.checked_add(1).ok_or(RawAllocError::SizeOverflow {
            count: self.len,
            stride: self.pair_layout.size(),
        })?;
        self.ensure_capacity(required)?;
        Ok(RawPairSlot {
            ptr: unsafe { self.slot_unchecked(self.len) },
        })
    }

    /// Return the key storage pointer for `slot`.
    #[must_use]
    pub fn key_ptr(&self, slot: RawPairSlot) -> *mut u8 {
        slot.ptr
    }

    /// Return the value storage pointer for `slot`.
    #[must_use]
    pub fn value_ptr(&self, slot: RawPairSlot) -> *mut u8 {
        unsafe { slot.ptr.add(self.value_offset) }
    }

    /// Mark the current next pair slot as initialized after the caller wrote
    /// its key and value.
    ///
    /// # Safety
    ///
    /// The caller must have just initialized the pair slot returned by
    /// [`next_uninit_pair`](Self::next_uninit_pair), and must call this at most
    /// once per reserved slot.
    pub unsafe fn mark_initialized(&mut self) {
        debug_assert!(self.len < self.cap, "marking beyond reserved capacity");
        self.len += 1;
    ...
    /// Drop an initialized value in a reserved pair slot that has not been
    /// marked initialized yet.
    ///
    /// # Safety
    ///
    /// The caller must have initialized the value storage for `slot`, must not
    /// have initialized or must separately handle the key storage, and must not
    /// call [`mark_initialized`](Self::mark_initialized) for this slot later.
    pub unsafe fn drop_unmarked_value(&mut self, slot: RawPairSlot) {
        unsafe {
            (self.value_drop)(self.value_drop_ctx, self.value_ptr(slot));
        }
    }

    /// Mark initialized pairs as adopted by the caller.
    pub fn adopt(&mut self) {
        self.len = 0;
    }

    unsafe fn slot_unchecked(&self, index: usize) -> *mut u8 {
        unsafe { self.ptr.add(index * self.pair_layout.size()) }
    }
    ...
    fn ensure_capacity(&mut self, required: usize) -> Result<(), RawAllocError> {
        ...
        let next_cap = required.max(doubled).max(4);
        let new_layout = array_layout(next_cap, self.pair_layout)?;

        if self.pair_layout.size() == 0 {
            self.cap = next_cap;
            return Ok(());
        }
...
impl Drop for RawPairBuilder {
    fn drop(&mut self) {
        for index in (0..self.len).rev() {
            let slot = unsafe { self.slot_unchecked(index) };
            unsafe {
                (self.value_drop)(self.value_drop_ctx, slot.add(self.value_offset));
                (self.key_drop)(self.key_drop_ctx, slot);
            }
        }
...
mod tests {
    ...
    #[test]
    fn raw_pair_builder_drops_initialized_pairs() {
        let key_drops = AtomicUsize::new(0);
        let value_drops = AtomicUsize::new(0);
        let key_ctx = &key_drops as *const AtomicUsize as *const ();
        let value_ctx = &value_drops as *const AtomicUsize as *const ();
        {
            let mut builder = RawPairBuilder::new(
                Layout::new::<(u32, u64)>(),
                core::mem::offset_of!((u32, u64), 1),
                key_ctx,
                count_drop,
                value_ctx,
                count_drop,
            );
            let slot = builder.next_uninit_pair().unwrap();
            unsafe {
                builder.key_ptr(slot).cast::<u32>().write(7);
                builder.value_ptr(slot).cast::<u64>().write(11);
                builder.mark_initialized();
            }
        }
    ...
    #[test]
    fn raw_pair_builder_adopt_disarms_values_but_keeps_allocation_owned() {
        let key_drops = AtomicUsize::new(0);
        let value_drops = AtomicUsize::new(0);
        let key_ctx = &key_drops as *const AtomicUsize as *const ();
        let value_ctx = &value_drops as *const AtomicUsize as *const ();
    ...
    #[test]
    fn raw_pair_builder_can_drop_unmarked_value() {
        let key_drops = AtomicUsize::new(0);
        let value_drops = AtomicUsize::new(0);
        let key_ctx = &key_drops as *const AtomicUsize as *const ();
        let value_ctx = &value_drops as *const AtomicUsize as *const ();
