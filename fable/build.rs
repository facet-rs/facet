//! Generate the fable typed AST — and its lowering — FROM THE GRAMMAR at build time.
//!
...
//!
//! Output: `$OUT_DIR/fable_ast.rs` (types + ResolvedCstNode lowering, `include!`d by
//! src/lib.rs) and `$OUT_DIR/fable_grammar.json` (embedded so the runtime parser needs
...
fn main() {
    ...
    fs::write(out.join("fable_grammar.json"), &grammar_json).expect("write embedded grammar");

    let ann_src = fs::read_to_string(&ann_js).expect("read annotation source");
    let ann_json =
        snark_dsl::annotations_from_source(&ann_src, "fable_ast.snark.js").expect("annotations");
    let raw =
    ...
    fs::write(out.join("fable_ast.rs"), generated).expect("write generated ast");
}
...
/// fields inside field content are out of scope for fable (and unused by the grammar).
...
fn field_alts(content: &RawRuleJson) -> Vec<Alt> {
    match unwrap_transparent(content) {
        ...
        other => panic!("unsupported field content in fable grammar: {other:?}"),
    }
}
...
impl<'a> Model<'a> {
    ...
    fn generate(&self) -> String {
        ...
        out.push_str(
            "// @generated by fable/build.rs from the fable grammar + fable_ast.snark.js.\n\
