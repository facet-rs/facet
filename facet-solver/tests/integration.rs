...
// ============================================================================
// Miette Diagnostics for Ambiguous Cases
// ============================================================================
...
struct DiagnosticDeserializer<'a> {
    data: &'a [u8],
    source_name: String,
}

impl<'a> DiagnosticDeserializer<'a> {
    fn new(data: &'a [u8], source_name: impl Into<String>) -> Self {
        Self {
            data,
    ...
    /// Find the byte offset of a key in the JSON source
    fn find_key_span(&self, key: &str) -> Option<SourceSpan> {
        let json_str = String::from_utf8_lossy(self.data);
        let key_pattern = format!("\"{key}\"");
        json_str
            .find(&key_pattern)
            .map(|idx| SourceSpan::from((idx, key_pattern.len())))
    }
    ...
    fn probe_for_config<'s>(
        ...
        loop {
            let event = match parser.parse_next() {
                Ok(e) => e,
                Err(_) => {
                    return Err(Box::new(NoMatchError {
                        src: NamedSource::new(
                            &self.source_name,
                            String::from_utf8_lossy(self.data).into_owned(),
                        ),
                        span: None,
                        help: "JSON parsing failed".into(),
                    }));
                }
            };
                    ...
                    match solver.probe_key(&path_refs, &key_str) {
                        ...
                        ProbeResult::NoMatch => {
                            return Err(Box::new(NoMatchError {
                                ...
                                span: self.find_key_span(&key_str),
                                help: format!(
                                    "Field '{key_str}' at path {path_refs:?} doesn't exist in any remaining configuration"
                                ),
                            }));
                        }
        ...
        // Still ambiguous at end of input
        ...
        if candidates.is_empty() {
            return Err(Box::new(NoMatchError {
                ...
                span: None,
                help: "No configuration matches the provided fields".into(),
            }));
        }

        // Build candidate info for the diagnostic
        let candidate_infos: Vec<CandidateInfo> = candidates
            .iter()
            .map(|c| {
                let fields: Vec<_> = c.fields().keys().copied().collect();
                let unique_fields: Vec<_> = fields
                    .iter()
                    .filter(|f| {
                        // Find fields unique to this config
                        candidates
                            .iter()
                            .filter(|other| other.field(f).is_some())
                            .count()
                            == 1
                    })
                    .copied()
                ...
                CandidateInfo {
                    ...
                    help: if unique_fields.is_empty() {
                        ...
                        format!("Unique fields that would disambiguate: {unique_fields:?}")
                    },
                }
            })
            .collect();

        Err(Box::new(AmbiguousJsonError {
            src: NamedSource::new(
                &self.source_name,
                String::from_utf8_lossy(self.data).into_owned(),
            ),
            span: SourceSpan::from((0, self.data.len())),
...
/// Two variants with IDENTICAL field names - impossible to disambiguate!
#[derive(Facet, Debug)]
struct IdenticalVariantA {
    x: i32,
    y: i32,
}

#[derive(Facet, Debug)]
struct IdenticalVariantB {
    x: i32, // Same!
    y: i32, // Same!
}
...
#[derive(Facet, Debug)]
struct IdenticalWrapper {
    #[facet(flatten)]
    inner: IdenticalEnum,
}

#[test]
fn test_ambiguous_identical_fields() {
    init_tracing();
    let _span = info_span!("test_ambiguous_identical_fields").entered();

    // Both variants have exactly x and y - impossible to tell apart!
    let json = br#"{"x": 1, "y": 2}"#;
    ...
    let deserializer = DiagnosticDeserializer::new(json, "ambiguous.json");
    let result = deserializer.probe_for_config(&schema);

    match result {
        Ok(_) => panic!("Should have failed with ambiguity!"),
        Err(diagnostic) => {
            // Print the beautiful diagnostic!
            let report = miette::Report::new_boxed(diagnostic);
            eprintln!("\n{report:?}\n");
        }
    }
}

/// Partially overlapping fields - one unique field could disambiguate
#[derive(Facet, Debug)]
struct OverlapA {
    common: String,
    a_specific: i32,
...
#[test]
fn test_ambiguous_only_common_fields() {
    init_tracing();
    let _span = info_span!("test_ambiguous_only_common_fields").entered();

    // Only provides `common` - both variants have this field!
    let json = br#"{"common": "hello"}"#;
    info!(json = %String::from_utf8_lossy(json), "input JSON (only common field)");
    ...
    let deserializer = DiagnosticDeserializer::new(json, "overlap.json");
    let result = deserializer.probe_for_config(&schema);

    match result {
        Ok(_) => panic!("Should have failed with ambiguity - only common field provided!"),
        Err(diagnostic) => {
            // Print the beautiful diagnostic with hints about unique fields
            let report = miette::Report::new_boxed(diagnostic);
            eprintln!("\n{report:?}\n");
        }
    }
...
/// Unknown field error with nice source location
#[test]
fn test_unknown_field_diagnostic() {
    init_tracing();
    let _span = info_span!("test_unknown_field_diagnostic").entered();

    // `typo_field` doesn't exist in any variant
    let json = br#"{"common": "hi", "typo_field": 42}"#;
    info!(json = %String::from_utf8_lossy(json), "input JSON (has typo)");

    let schema = Schema::build(OverlapWrapper::SHAPE).unwrap();
    let deserializer = DiagnosticDeserializer::new(json, "typo.json");
    let result = deserializer.probe_for_config(&schema);

    match result {
        Ok(_) => panic!("Should have failed with unknown field!"),
        Err(diagnostic) => {
            let report = miette::Report::new_boxed(diagnostic);
            eprintln!("\n{report:?}\n");
        }
...
/// Three-way ambiguity - even more configurations to show
#[derive(Facet, Debug)]
struct ThreeA {
    shared: String,
}
...
fn test_three_way_ambiguity() {
    init_tracing();
    let _span = info_span!("test_three_way_ambiguity").entered();

    let json = br#"{"shared": "value"}"#;
    ...
    match result {
        Ok(_) => panic!("Should have failed with 3-way ambiguity!"),
        Err(diagnostic) => {
            let report = miette::Report::new_boxed(diagnostic);
            eprintln!("\n{report:?}\n");
        }
