...
//! API wraps the engine. When Weavy's single `jit` feature and the target platform
//! enable native copy-and-patch, the typed API routes through `phon-jit` for
//! supported lowered programs. Otherwise it runs the `phon-engine` interpreter -
