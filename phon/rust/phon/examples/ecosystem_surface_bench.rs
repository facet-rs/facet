//! Ecosystem-shaped typed codec benchmarks for the Vox/Phon roadmap.
//!
//! These payloads mirror the checked-in Rust fixture families in
//! `rust/phon/tests/ecosystem_surface.rs`: Dodeca containers/dynamic values,
//! Dibs SQL rows, Styx and Stax recursion, Helix trace-viewer snapshots,
//! Hotmeal live reload, and Tracey migration DTOs.
//!
//! The interpreter baseline uses `phon-engine` directly. The "selected runtime"
//! path uses `phon::api::Codec`, which is the public Phon front door Vox should
//! route through: when the native JIT supports a lowered program it runs native
//! code, otherwise it falls back to the interpreter and reports why.
//!
//! Run: `cargo run -p phon --release --features jit --example ecosystem_surface_bench`
...
#[derive(Debug, Clone, PartialEq, Facet)]
struct DodecaAssetInfo {
    jxl_srcset: Vec<(String, u32)>,
    webp_srcset: Vec<(String, u32)>,
}

#[derive(Debug, Clone, PartialEq, Facet)]
struct DodecaHtmlPayload {
    html: String,
    path_map: HashMap<String, String>,
    known_routes: HashSet<String>,
    code_metadata: HashMap<String, DodecaMetadata>,
    asset_info: HashMap<String, DodecaAssetInfo>,
...
struct TraceyDataUpdate {
    version: u64,
    newly_covered: Vec<TraceyRuleId>,
    newly_uncovered: Vec<TraceyRuleId>,
}

#[derive(Debug, Clone, PartialEq, Facet)]
struct TraceyDashboardPayload {
    spec: String,
    impl_name: String,
    total_rules: usize,
    covered_rules: usize,
    ...
    update: Option<TraceyDataUpdate>,
}
...
{
    let derived = phon::derive::of::<T>().expect("ecosystem bench shape should derive");
    let reg = Registry::new(derived.schemas.clone());
    typed::lower_typed(&derived.descriptor, &derived.descriptor_blocks, &reg)
        .expect("ecosystem bench shape should lower")
}
...
fn bench(label: &str, iters: u64, mut f: impl FnMut()) -> f64 {
    ...
    println!("  {label:<26} {ns:>10.1} ns/op");
    ns
}

fn print_fallbacks<T>(codec: &Codec<T>)
where
    T: for<'facet> Facet<'facet>,
{
    let report = codec.jit_fallback_report();
    if report.is_empty() {
        println!("  fallback report            native-clean");
        return;
    }

    let decode = report.decode.len();
    let encode = report.encode.len();
    println!("  fallback report            decode={decode} encode={encode}");
    for record in report.decode.iter().chain(report.encode.iter()).take(4) {
        println!("    {}: {}", record.path, record.reason);
    }
}

fn bench_case<T>(label: &str, value: T, iters: u64)
where
    T: for<'facet> Facet<'facet> + PartialEq + Debug,
{
    let lowered = lower::<T>();
    let base = core::ptr::from_ref(&value).cast::<u8>();
    let interp_wire = unsafe { typed::encode_with(&lowered, base) };
    let interp_decoded = decode_interp::<T>(&lowered, &interp_wire);
    assert_eq!(
        interp_decoded, value,
        "{label}: interpreter round-trip mismatch"
    );

    let codec = Codec::<T>::new().expect("ecosystem bench codec should build");
    let selected_wire = codec
        .encode(&value)
        .expect("selected runtime encode should succeed");
    assert_eq!(
        selected_wire, interp_wire,
        "{label}: selected runtime encode mismatch"
    );
    let selected_decoded = codec
        .decode(&selected_wire)
        .expect("selected runtime decode should succeed");
    assert_eq!(
        selected_decoded, value,
        "{label}: selected runtime decode mismatch"
    );

    println!("{label}  ->  {} wire bytes", interp_wire.len());
    ...
    let dec_i = bench("decode interpreter", iters, || {
        let decoded = decode_interp::<T>(&lowered, &interp_wire);
        black_box(decoded);
    });
    let enc_s = bench("encode selected runtime", iters, || {
        let bytes = codec.encode(&value).expect("selected encode");
        black_box(bytes);
    });
    let dec_s = bench("decode selected runtime", iters, || {
        let decoded = codec.decode(&interp_wire).expect("selected decode");
        black_box(decoded);
    });

    println!(
        "  selected/interpreter       encode {:>5.2}x   decode {:>5.2}x",
        enc_i / enc_s,
        dec_i / dec_s
    );
    print_fallbacks(&codec);
    println!();
}
...
fn make_dodeca_html() -> DodecaHtmlPayload {
    ...
    for i in 0..48 {
        path_map.insert(format!("/old/{i}.css"), format!("/assets/{i}.css"));
        known_routes.insert(format!("/guide/{i}/"));
        code_metadata.insert(
            format!("sample-{i}.rs"),
            DodecaMetadata {
                language: "rust".to_string(),
                dependencies: vec!["facet".to_string(), "phon".to_string()],
                duration_ms: 8 + i as u64,
            },
        );
        asset_info.insert(
            format!("/image-{i}.png"),
            DodecaAssetInfo {
                jxl_srcset: vec![(format!("/image-{i}-640.jxl"), 640)],
                webp_srcset: vec![(format!("/image-{i}-640.webp"), 640)],
            },
        );
        vite_css_map.insert(
            format!("/entry-{i}.ts"),
            vec![
                format!("/assets/entry-{i}.css"),
                format!("/assets/chunk.css"),
            ],
        );
    }

    DodecaHtmlPayload {
        html: "<main><h1>Phon migration</h1></main>".repeat(12),
        path_map,
        known_routes,
        code_metadata,
        asset_info,
...
fn make_dodeca_template_call() -> DodecaTemplateCall {
    DodecaTemplateCall {
        context_id: "ctx-ecosystem".to_string(),
        name: "render_section".to_string(),
        args: (0..16).map(make_dynamic_object).collect(),
        kwargs: (0..8)
            .map(|i| (format!("kwarg_{i}"), make_dynamic_object(i + 32)))
            .collect(),
    }
}

fn make_dodeca_load_data_result() -> DodecaLoadDataResult {
    DodecaLoadDataResult::Success {
        value: make_dynamic_object(42),
    }
}

fn make_dodeca_frontmatter_extra() -> Value {
    let mut object = VObject::new();
...
fn make_dodeca_source_kind(i: u32) -> DodecaSourceKind {
    match i % 14 {
        0 => DodecaSourceKind::Heading,
        1 => DodecaSourceKind::Paragraph,
        2 => DodecaSourceKind::BlockQuote,
        3 => DodecaSourceKind::List,
        4 => DodecaSourceKind::ListItem,
        5 => DodecaSourceKind::DefinitionList,
        6 => DodecaSourceKind::DefinitionListTitle,
        7 => DodecaSourceKind::DefinitionListDefinition,
        8 => DodecaSourceKind::ThematicBreak,
        9 => DodecaSourceKind::Table,
        10 => DodecaSourceKind::TableHead,
        11 => DodecaSourceKind::TableRow,
        12 => DodecaSourceKind::TableCell,
        _ => DodecaSourceKind::Image,
    }
}

fn make_dodeca_parse_result() -> DodecaParseResult {
    let entries = (0..64)
        .map(|i| DodecaSourceMapEntry {
            id: format!("node-{i}"),
            kind: make_dodeca_source_kind(i),
            line_start: 5 + i,
            line_end: 5 + i,
            byte_start: u64::from(i) * 24,
            byte_end: u64::from(i) * 24 + 18,
        })
        .collect();
    ...
    DodecaParseResult::Success {
        frontmatter: DodecaFrontmatter {
            ...
            extra: make_dodeca_frontmatter_extra(),
        },
        html: "<h1 data-sid=\"h1\">Intro</h1><p data-sid=\"p1\">Generated fixture</p>".repeat(12),
        headings: (0..16)
            .map(|i| DodecaMarkdownHeading {
            ...
            .collect(),
        reqs: (0..8)
            .map(|i| DodecaReqDefinition {
                id: format!("vox.dodeca.markdown.{i}"),
                anchor_id: format!("r-vox-dodeca-markdown-{i}"),
            })
            .collect(),
        head_injections: vec![
            "<link rel=\"stylesheet\" href=\"/assets/arborium.css\">".to_string(),
            "<script type=\"module\" src=\"/assets/search.js\"></script>".to_string(),
        ],
...
fn make_dodeca_decoded_image(seed: u8, width: u32, height: u32) -> DodecaDecodedImage {
...
fn make_dodeca_image_processor_fixture() -> DodecaImageProcessorFixture {
    let decoded = make_dodeca_decoded_image(0x20, 96, 64);
    let resized = make_dodeca_decoded_image(0x80, 48, 32);
    DodecaImageProcessorFixture {
        png_data: (0..16_384).map(|i| (i & 0xff) as u8).collect(),
...
fn make_dodeca_search_indexer_fixture() -> DodecaSearchIndexerFixture {
    let pages = (0..32)
        .map(|i| DodecaSearchPage {
            url: format!("/guide/{i}/"),
            source: "docs".to_string(),
            html: format!(
                "<main><h1>Page {i}</h1><p>Phon typed plan search content {i}.</p></main>"
            )
            .repeat(4),
        })
        .collect();
    let files = (0..8)
        .map(|i| DodecaSearchFile {
            path: format!("/search/chunk-{i}"),
            contents: (0..1024).map(|j| ((i * 31 + j) & 0xff) as u8).collect(),
        })
        .collect();
    ...
    let rows = (0..64)
        .map(|i| {
            vec![
                DibsRowField {
                    name: "id".to_string(),
                    value: SqlValue::I64(i),
                },
                DibsRowField {
                    name: "name".to_string(),
                    value: SqlValue::String(format!("row-{i}")),
                },
                DibsRowField {
                    name: "blob".to_string(),
                    value: SqlValue::Bytes(vec![i as u8; 32]),
                },
            ]
        })
        .collect();
...
fn make_dibs_service_row(i: u32) -> DibsRow {
    DibsRow {
        fields: vec![
            DibsRowField {
                name: "id".to_string(),
...
fn make_dibs_schema_info() -> DibsSchemaInfo {
    DibsSchemaInfo {
        tables: vec![DibsTableInfo {
...
fn make_dibs_squel_service_fixture() -> DibsSquelServiceFixture {
    let row = make_dibs_service_row(7);
    DibsSquelServiceFixture {
        schema: make_dibs_schema_info(),
        list_request: DibsListRequest {
        ...
        list_response: DibsListResult::Ok(DibsServiceListResponse {
            rows: vec![row.clone(), make_dibs_service_row(8)],
            total: Some(2),
        }),
        get_request: DibsGetRequest {
            table: "products".to_string(),
...
fn make_dibs_migration_service_fixture() -> DibsMigrationServiceFixture {
...
fn styx_scalar(text: String, kind: StyxScalarKind, start: u32, end: u32) -> StyxValue {
    StyxValue {
        tag: None,
...
fn make_styx_value(depth: u32) -> StyxValue {
    if depth == 0 {
        return styx_scalar("leaf".to_string(), StyxScalarKind::Bare, 1, 5);
    }

    StyxValue {
        tag: Some(StyxTag {
            name: format!("object-{depth}"),
            span: styx_span(depth, depth + 7),
        }),
        payload: Some(StyxPayload::Object(StyxObject {
            entries: vec![StyxEntry {
                key: make_styx_value(0),
                value: StyxValue {
                    tag: None,
                    payload: Some(StyxPayload::Sequence(StyxSequence {
                        items: vec![make_styx_value(depth - 1), make_styx_value(0)],
                        span: styx_span(depth + 8, depth + 16),
                    })),
                    span: None,
                },
                doc_comment: Some("benchmark".to_string()),
            }],
            span: styx_span(depth + 7, depth + 20),
        })),
        span: Some(StyxSpan {
            start: depth,
            end: depth + 10,
...
fn make_styx_lsp_surface() -> StyxLspSurfaceFixture {
    let uri = "file:///workspace/queries.styx".to_string();
    let source = "@query { from products select (id name) }".to_string();
    let value = make_styx_value(3);
    let cursor = StyxLspCursor {
        line: 0,
        character: 16,
        offset: 16,
    };
    let range = StyxLspRange {
        start: StyxLspPosition {
            line: 0,
            character: 0,
        },
    ...
    let diagnostic = StyxLspDiagnostic {
    ...
    StyxLspSurfaceFixture {
        ...
        completion_params: StyxLspCompletionParams {
            ...
            tagged_context: Some(value.clone()),
        },
        completions: vec![
        ...
        hover_result: Some(StyxLspHoverResult {
...
fn make_stax_node(depth: u32, fanout: u32) -> FlameNode {
    let children = if depth == 0 {
        Vec::new()
    } else {
        (0..fanout)
            .map(|_| make_stax_node(depth - 1, fanout))
            .collect()
    };
    FlameNode {
        address: 0x1000 + depth as u64,
        function_name: Some(depth),
        on_cpu_ns: 1_000 + depth as u64,
        children,
    }
}

fn make_stax_flamegraph() -> StaxFlamegraph {
    StaxFlamegraph {
        total_on_cpu_ns: 100_000,
        strings: (0..64).map(|i| format!("symbol-{i}")).collect(),
        root: make_stax_node(4, 3),
    }
}

fn make_stax_linux_broker_control() -> StaxLinuxBrokerControlFixture {
...
fn make_stax_mac_record_fixture() -> StaxMacRecordFixture {
    let records = (0..128)
        .map(|i| StaxMacKdBuf {
            timestamp: 900_000 + i * 128,
            arg1: 0x1000 + i,
            arg2: 0x2000 + i,
            arg3: 0x3000 + i,
            arg4: 0x4000 + i,
            arg5: 0xfeed_face,
            debugid: 0x3101_0004 + (i as u32 & 0xf),
            cpuid: i as u32 % 8,
            unused: 0,
        })
        .collect();
    ...
    StaxMacRecordFixture {
        config: StaxMacSessionConfig {
            ...
            pmu_event_configs: vec![0xfeed_beef, 0x1_0000_0001, 0x2_0000_0002],
            class_mask: 0b1011,
        ...
        batch: StaxMacKdBufBatch {
            ...
            drained_mach_ticks: 916_384,
            queued_for_send_mach_ticks: 916_420,
            send_started_mach_ticks: 916_480,
...
fn make_helix_trace() -> HelixTraceSnapshot {
    let surface = sample_helix_trace_service_surface();
    HelixTraceSnapshot {
        meta: surface.meta,
        run_info: surface
            .run_info
            .expect("trace-service sample should include run info"),
        rollup: surface.pulse_rollup,
        prompt_layout: surface.prompt_layout,
        attention_heatmap: surface.pulse_attention_heatmap,
        stream_metrics: surface.stream_metrics,
        pulse_available: surface.pulse_available,
    }
}

fn make_hotmeal_batch() -> HotmealLiveReloadBatch {
    HotmealLiveReloadBatch {
        route: "/guide/".to_string(),
        events: vec![
            HotmealLiveReloadEvent::Reload,
            HotmealLiveReloadEvent::Patches {
                route: "/guide/".to_string(),
                patches_blob: vec![0xab; 2048],
            },
...
fn make_tracey_dashboard() -> TraceyDashboardPayload {
    let rules = (0..48)
        .map(|i| TraceyRuleRef {
            id: TraceyRuleId {
                base: format!("compat.rule-{i}"),
                version: 1,
            },
            text: Some(format!("Rule text {i}")),
        })
        .collect();
    TraceyDashboardPayload {
        spec: "phon".to_string(),
        ...
        update: Some(TraceyDataUpdate {
            version: 42,
            newly_covered: vec![TraceyRuleId {
                base: "validate.uniqueness".to_string(),
                version: 1,
            }],
            newly_uncovered: vec![TraceyRuleId {
...
fn make_dodeca_byte_channel_item() -> Vec<u8> {
    (0..4096).map(|i| ((i * 31 + 7) & 0xff) as u8).collect()
}

fn make_dodeca_string_channel_item() -> String {
    (0..64)
        .map(|i| format!("diagnostic-{i}: route /guide/topic-{i}/ refreshed"))
        .collect::<Vec<_>>()
        .join("\n")
}

fn make_dibs_migration_log() -> DibsMigrationLog {
    DibsMigrationLog {
        level: DibsLogLevel::Info,
        message: "Applied 202606051142_add_dibs_admin_views (38ms)".to_string(),
        migration: Some("202606051142_add_dibs_admin_views".to_string()),
...
fn make_helix_pulse_available() -> HelixPulseAvailable {
    HelixPulseAvailable {
        pulse_id: HelixSchedulerPulseId(9_001),
    }
}

fn make_tracey_data_update() -> TraceyDataUpdate {
    TraceyDataUpdate {
        version: 42,
        newly_covered: (0..32)
            .map(|i| TraceyRuleId {
...
fn main() {
    println!("Vox ecosystem typed codec steady-state throughput\n");
    bench_case("dodeca(html maps/sets)", make_dodeca_html(), 30_000);
    bench_case(
        "dodeca(dynamic template)",
        make_dodeca_template_call(),
        40_000,
    );
    bench_case(
        "dodeca(load data dynamic result)",
        make_dodeca_load_data_result(),
        80_000,
    );
    bench_case(
        "dodeca(parse result boxed source map)",
        make_dodeca_parse_result(),
        30_000,
    );
    bench_case(
        "dodeca(image processor roots)",
        make_dodeca_image_processor_fixture(),
        20_000,
    );
    bench_case(
    ...
    bench_case(
        "dibs(squel service roots)",
        make_dibs_squel_service_fixture(),
        30_000,
    );
    bench_case(
        "dibs(migration service roots)",
        make_dibs_migration_service_fixture(),
        50_000,
    );
    bench_case("styx(recursive value)", make_styx_value(5), 80_000);
    bench_case("styx(lsp surface)", make_styx_lsp_surface(), 30_000);
    bench_case("stax(flamegraph)", make_stax_flamegraph(), 20_000);
    bench_case(
        "stax(linux fd control)",
        make_stax_linux_broker_control(),
        80_000,
    );
    bench_case(
        "stax(mac kdbuf batches)",
        make_stax_mac_record_fixture(),
        40_000,
    );
    bench_case("helix(trace snapshot)", make_helix_trace(), 30_000);
    bench_case(
        "helix(trace service aggregate)",
        sample_helix_trace_service_surface(),
        5_000,
    );
    bench_case("hotmeal(live reload)", make_hotmeal_batch(), 120_000);
    bench_case("tracey(migration dto)", make_tracey_dashboard(), 50_000);
    println!("Channel item typed codec steady-state throughput\n");
    bench_case(
        "dodeca(byte tunnel channel item)",
        make_dodeca_byte_channel_item(),
        80_000,
    );
    bench_case(
        "dodeca(lsp string channel item)",
        make_dodeca_string_channel_item(),
        80_000,
    );
    bench_case(
        "dibs(migration log channel item)",
        make_dibs_migration_log(),
        80_000,
    );
    bench_case(
        "helix(pulse available channel item)",
        make_helix_pulse_available(),
        120_000,
    );
    bench_case(
        "tracey(data update channel item)",
        make_tracey_data_update(),
        50_000,
    );
}
