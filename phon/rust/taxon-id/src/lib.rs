...
// ============================================================================
//
// The byte primitives live in `crate::sink`; every tag and marker token is fed
...
fn visit_kind_targets(kind: &Kind, f: &mut impl FnMut(SchemaId)) {
    ...
    match kind {
        ...
        | Kind::Tensor { element, .. }
        | Kind::Option { element }
        | Kind::Channel { element, .. } => on_ref(element, f),
        Kind::Map { key, value } => {
            on_ref(key, f);
            on_ref(value, f);
        }
...
impl Walk<'_> {
    ...
    fn schema<S: Sink>(&self, idx: NodeIx, path: &[NodeIx], out: &mut S) {
        ...
        match &schema.kind {
            ...
            Kind::Channel { direction, element } => {
                write_str(out, "channel");
                write_str(out, direction_tag(*direction));
                self.reference(element, path, out);
            }
            Kind::Dynamic => write_str(out, "dynamic"),
            Kind::External { kind, metadata } => {
...
fn remap_kind(kind: &Kind, map: &HashMap<u64, SchemaId>) -> Kind {
    match kind {
        ...
        Kind::Struct { name, fields } => Kind::Struct {
            name: name.clone(),
            fields: fields.iter().map(|f| remap_field(f, map)).collect(),
        },
        Kind::Enum { name, variants } => Kind::Enum {
            name: name.clone(),
            variants: variants
                .iter()
                .map(|v| Variant {
        ...
        Kind::Tuple { elements } => Kind::Tuple {
            elements: elements.iter().map(|r| remap_ref(r, map)).collect(),
        },
        Kind::List { element } => Kind::List {
            element: remap_ref(element, map),
        },
        Kind::Set { element } => Kind::Set {
            element: remap_ref(element, map),
        },
        Kind::Option { element } => Kind::Option {
            element: remap_ref(element, map),
        },
        ...
        Kind::Tensor { element, rank } => Kind::Tensor {
            element: remap_ref(element, map),
            rank: *rank,
        },
        Kind::Channel { direction, element } => Kind::Channel {
            direction: *direction,
            element: remap_ref(element, map),
        },
...
mod tests {
    ...
    fn required_flag_is_part_of_identity() {
        ...
        if let Kind::Struct { fields, .. } = &mut optional.kind {
    ...
    fn recursive_schema_terminates_and_is_order_independent() {
        ...
        let id_of = |schemas: &[Schema], want_struct: bool| -> SchemaId {
            schemas
                .iter()
                .find(|s| matches!(&s.kind, Kind::Struct { .. }) == want_struct)
                .unwrap()
                .id
        };
        ...
        let node = forward
            .iter()
            .find(|s| matches!(&s.kind, Kind::Struct { .. }))
            .unwrap();
        if let Kind::Struct { fields, .. } = &node.kind {
