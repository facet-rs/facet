...
export function readWorkspaceVersion(cargoTomlSource) {
  ...
  if (!match?.[1]) {
    throw new Error("Could not determine workspace version from rust/Cargo.toml");
  }
  return match[1];
}
...
export function syncTypeScriptPackageVersions(repoRoot) {
  const cargoTomlPath = join(repoRoot, "rust", "Cargo.toml");
...
if (entrypoint === fileURLToPath(import.meta.url)) {
  ...
  if (updatedPackages.length === 0) {
    console.log(`TypeScript package versions already match Rust workspace version ${version}`);
  } else {
