...
public func assertTypedEquivalence<T>(
    ...
    let encodeLowered = try lowerTyped(descriptor, registry, blocks)
    let decodeLowered = try lowerDecode(root, descriptor, registry, blocks)

    // Reference bytes from the first engine.
    let ref = withUnsafeBytes(of: value) {
        try! engines[0].compileEncode(encodeLowered)($0.baseAddress!)
    }
    ...
    for e in engines {
        let enc = try e.compileEncode(encodeLowered)
        ...
        let dec = try e.compileDecode(decodeLowered)
...
public func nativeFallbackReport(
    descriptor: Descriptor,
    registry: Registry,
    blocks: [SchemaId: Descriptor] = [:]
) throws -> JitFallbackReport {
    try PhonJIT.nativeFallbackReport(lowerTyped(descriptor, registry, blocks))
}
