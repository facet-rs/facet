...
/// identically (an independent oracle needing no per-type `T → Value` bridge), and
/// (3) every engine round-trips back to `value`. Written once; every backend runs it.
public func assertTypedEquivalence<T: Equatable>(
    ...
    for e in engines {
        let bytes = withUnsafeBytes(of: &v) {
            try! e.compileEncode(descriptor, registry)($0.baseAddress!)
        }
