...
fn test_deserialize_string_to_string_map() {
    ...
    let map: HashMap<String, String> = facet_yaml_legacy::from_str(yaml).unwrap();
    assert_eq!(map.len(), 3);
...
fn test_deserialize_string_to_u64_map() {
    ...
    let map: HashMap<String, u64> = facet_yaml_legacy::from_str(yaml).unwrap();
    assert_eq!(map.len(), 3);
...
fn test_deserialize_nested_maps() {
    ...
    let map: HashMap<String, HashMap<String, String>> = facet_yaml_legacy::from_str(yaml).unwrap();
    assert_eq!(map.len(), 2);
