...
fn test_transparent_string() {
    ...
    let my_string: MyString = facet_yaml_legacy::from_str(yaml).unwrap();
...
fn test_transparent_string_vec() {
    ...
    let strings: Vec<MyString> = facet_yaml_legacy::from_str(yaml).unwrap();
    assert_eq!(
        strings,
        vec![
...
fn test_transparent_in_struct() {
    ...
    let message: Message = facet_yaml_legacy::from_str(yaml).unwrap();
    assert_eq!(
        message,
        Message {
