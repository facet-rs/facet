...
fn test_untagged_scalar_string_vs_int() {
    ...
    // Integer value
    let int_val: StringOrInt = facet_yaml::from_str("42").unwrap();
    assert_eq!(int_val, StringOrInt::Int(42));

    // Negative integer
    let neg_val: StringOrInt = facet_yaml::from_str("-100").unwrap();
    assert_eq!(neg_val, StringOrInt::Int(-100));

    // String value
    let str_val: StringOrInt = facet_yaml::from_str("hello").unwrap();
    assert_eq!(str_val, StringOrInt::Str("hello".to_string()));

    // Quoted string that looks like a number
    let quoted: StringOrInt = facet_yaml::from_str("\"42\"").unwrap();
...
fn test_untagged_scalar_numeric_specificity() {
    ...
    let small: NumericValue = facet_yaml::from_str("42").unwrap();
    assert_eq!(small, NumericValue::Small(42));

    // Value too large for u8 but fits in u16
    let medium: NumericValue = facet_yaml::from_str("1000").unwrap();
    assert_eq!(medium, NumericValue::Medium(1000));

    // Value too large for u16 but fits in u32
    let large: NumericValue = facet_yaml::from_str("100000").unwrap();
...
fn test_untagged_scalar_signed_vs_unsigned() {
    ...
    let pos: SignedOrUnsigned = facet_yaml::from_str("100").unwrap();
    assert_eq!(pos, SignedOrUnsigned::Unsigned(100));

    // Negative value must go to signed
    let neg: SignedOrUnsigned = facet_yaml::from_str("-100").unwrap();
...
fn test_untagged_scalar_float_vs_int() {
    ...
    // Integer value
    let int_val: FloatOrInt = facet_yaml::from_str("42").unwrap();
    assert_eq!(int_val, FloatOrInt::Int(42));

    // Float value (has decimal point)
    let float_val: FloatOrInt = facet_yaml::from_str("3.5").unwrap();
...
value: hello
"#;
    let config2: Config = facet_yaml::from_str(yaml2).unwrap();
...
fn test_untagged_tuple_basic() {
    ...
    let point: Point = facet_yaml::from_str("[1.0, 2.0]").unwrap();
...
fn test_untagged_tuple_different_arities() {
    ...
    // Pair (arity 2)
    let pair: Data = facet_yaml::from_str("[1, 2]").unwrap();
    assert_eq!(pair, Data::Pair(1, 2));

    // Triple (arity 3)
    let triple: Data = facet_yaml::from_str("[1, 2, 3]").unwrap();
...
fn test_untagged_tuple_color() {
    ...
    let red: Color = facet_yaml::from_str("[255, 0, 0]").unwrap();
    assert_eq!(red, Color::Rgb(255, 0, 0));

    let white: Color = facet_yaml::from_str("[255, 255, 255]").unwrap();
...
position: [0.0, 0.0]
"#;
    let loc: Location = facet_yaml::from_str(yaml).unwrap();
...
- [3, 4]
"#;
    let points: Vec<Point> = facet_yaml::from_str(yaml).unwrap();
...
fn test_untagged_mixed_scalar_and_struct() {
    ...
    // Simple string value
    let simple: Config = facet_yaml::from_str("localhost:8080").unwrap();
    assert_eq!(simple, Config::Simple("localhost:8080".to_string()));

    // Detailed struct value
    let detailed: Config = facet_yaml::from_str("host: localhost\nport: 8080").unwrap();
    assert_eq!(
        detailed,
        Config::Detailed {
